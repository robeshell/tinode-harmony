...
> 仓库根 [README](../../README.md) 有 3 步上手指引；下面是逐项操作清单。
...
| 想试什么 | 怎么点 | 现象 |
| --- | --- | --- |
| 文本 / 富文本 | 输入框输入 → `B` / `I` → 发送 | 气泡按加粗/斜体渲染 |
| 表情 | 输入栏 😊 → 表情网格 → 点一下插入 → 发送 | 表情面板 + 消息气泡 |
| 引用 / 位置 | 加号 → 引用末条 / 位置 | 引用气泡 / 位置卡片（浅蓝地图区 + 定位图标 + 地址与坐标） |
| 已读 / 正在输入 / 撤回 | 顶部「已读」；加号 →「撤回末条」 | 位点推进；被撤那条变成居中「你撤回了一条消息」 |
| 历史分页 | 顶部「更早」 | 向前翻页加载 |
| 断线重连 | 关掉网络再打开，或断开再连 | 状态自动恢复、重连后自动重订阅 |
| **群聊：建群** | 会话列表顶栏「建群」→ 填群名 → 创建 | 自动进入新群；日志里能看到服务端把占位名 `new…` 换成真正的 `grp…` |
| **群聊：邀请成员** | 群聊页顶栏「成员」→ 填对方 `usrXXXXXX` → 「邀请 JRWPA」 | 成员列表 1 → 2 人；被邀请方重新进 App 就能看到这个群 |
| **群聊：群内收发** | 两边都进同一个群，各发一条 | 双方互见（真实服务端已验证：`examples/node/group-check.mjs` 17/17） |
| **群聊：移出成员** | 群聊页顶栏「成员」→ 对方那行点「移出」 | 成员列表少一人（自己那行不显示「移出」） |

**本机演示**（不接文件服务：消息会发出去，但文件内容不同步给对端）

| 想试什么 | 怎么点 | 现象 |
| --- | --- | --- |
| 图片 | 加号 → 图片 → 选图 → 完成 | 气泡显示缩略图，点击全屏预览 |
| 语音 | 切到 🎤 → 按住说话 → 松开 | 气泡 `▶ 1"`，点击播放（再点停止） |
| 文件 | 加号 → 文件 → 选文件 | 文件卡（名 + 大小），点击给出提示 |

**两个账号对聊**：`node examples/node/selfbootstrap.mjs` 注册第二个账号（会打印 `uid`），
在 App「新会话」输入框填对方的 `usrXXXXXX` → 打开 → 互发。
...
| 群里看不到群名，只显示 `grpXXXXXX` | 群名在群自己的 `desc.public.fn`（不是 `meta.sub[].pub`）；本示例已用 `groupInfo()` 取群名，若仍显示 id 说明该群没设群名 |
